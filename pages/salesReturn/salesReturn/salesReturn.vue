<template>
	<view class="background"  @click="hideIsShow">
		...
		<view class="container add">
			<navigator url="/pages/salesReturn/add/add">
			...
			<block v-for="(item,index) in list" :key='item.id'>
				<view class="list_box container">
					...
					<view class="list">
						...
						<view class="value">{{item.format_pay}}</view>
					</view>
					<view class="list">
					...
					<view class="list_click_box" :class="[isShow  == index ?'show' :'hide']">
						<view class="list_click_text list_click_del" @click="del(item.id)" v-if="item.pay == 1">删除</view>
						<view class="list_click_text list_click_del" style="color: #098;" v-if="item.pay == 1" @click="collection(item.id)">退款</view>
						<view class="list_click_text list_click_del" style="color: #FF5722;" v-if="item.pay == 2" @click="credentials(item.id)">查看凭证</view>
...
<script>
	export default {
		...
		methods: {
			courier(eve) {
				uni.navigateTo({
					url:'/pages/salesReturn/Courier/Courier?data='+encodeURIComponent(JSON.stringify(eve))
				})
			},
			sale(eve) {
				uni.navigateTo({
			...
			details(eve) {
				uni.navigateTo({
					url:'/pages/salesReturn/details/details?data='+encodeURIComponent(JSON.stringify(eve))
				})
			},
		}
	}
